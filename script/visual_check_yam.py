"""Spawn the YAM robot using the RoboTwin pipeline's own loading code.
...
Run from the RoboTwin root:
    uv run script/visual_check_yam.py
"""
...
# robot_file = _embodiment_types["franka-panda"]["file_path"]
robot_file = _embodiment_types["yam"]["file_path"]
...
# YAM is a single-arm embodiment (dual_arm: False), so two arms are spawned
# with distance embodiment_dis (default: 0.6m)
kwargs = {
...
# ── Print diagnostics ─────────────────────────────────────────────────
print("\n=== YAM Robot Loaded via RoboTwin Pipeline ===")
...
print(f"\n  Active joints (Left):  {[j.get_name() for j in robot.left_entity.get_active_joints()]}")
print(f"  Active joints (Right): {[j.get_name() for j in robot.right_entity.get_active_joints()]}")
print(f"  All links (Left):      {[l.get_name() for l in robot.left_entity.get_links()]}")
print(f"  All links (Right):     {[l.get_name() for l in robot.right_entity.get_links()]}")
